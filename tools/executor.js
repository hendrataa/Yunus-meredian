  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        emit("swap", { inputSymbol: args.input_mint?.slice(0, 8), outputSymbol: "SOL", amountIn: result.amount_in, amountOut: result.amount_out, tx: result.tx });
        ...
        emit("deploy", { pair: result.pool_name || args.pool_name, amountSol: args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx });
        stageSignals(args.pool_address, { pool: args.pool_address, deployed_at: Date.now() }).catch?.(() => {});
